  ...
  for (const record of records || []) {
    ...
    try {
      ...
      try {
        ...
        if (parts.length <= 1) {
          console.warn(` - Skip (Invalid storage path format: ${record.sanitized_document_path})`);
          continue;
        }
        ...
        if (downloadError) {
          console.warn(` - Skip (Download failed: ${downloadError.message})`);
          continue;
        }
        fileData = data;
      } catch (e: any) {
        console.error(` - Skip (Download error: ${e.message})`);
        continue;
      }
