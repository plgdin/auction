import { supabase } from "../../utils/common/storage.js";
...
export async function parseMetalMandiRates(scrapedRates?: ScrapedRate[] | string): Promise<MetalMandiRate[]> {
  ...
  if (Array.isArray(ratesToProcess) && ratesToProcess.length > 0) {
    ...
    for (const rate of ratesToProcess) {
      ...
      if (error) {
        console.error(`[MetalMandi] Failed to upsert rate for ${gradeName}:`, error.message);
      } else {
  ...
  // Fallback: If no scraped rates were provided or processing failed, check existing DB records or seed defaults
  console.log("[MetalMandi] Checking database for existing rates or applying baseline fallbacks...");
  ...
  if (existingRows && existingRows.length > 0) {
    return existingRows as MetalMandiRate[];
  }

  // Seed default legacy configuration into DB
  for (const item of legacyConfig) {
    const id = `metalmandi_${item.type}_${item.name.toLowerCase().replace(/[^a-z0-9]/g, '_')}_kg`;
    const payload: MetalMandiRate = {
      id,
      metal_type: item.type,
      grade_name: item.name,
      price_per_kg: item.defaultVal,
