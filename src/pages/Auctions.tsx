  ...
  if (item.raw_materials_text) {
    try {
      ...
      if (
        ...
        let preBidDdg =
          parsed.depositDetails.preBidDdg ||
          "Not required for registered MSME bidders";
      ...
      console.warn(
        "Failed to parse raw_materials_text as JSON, falling back to mock generator:",
        e,
      );
    }
  }
...
export function Auctions() {
  ...
  const activeTab =
    searchParams.get("tab") === "commercial" ? "commercial" : "mstc";
  ...
  const [selectedPreviewItem, setSelectedPreviewItem] =
    useState<MstcSanitizedAuction | null>(null);
  ...
  const selectedMstcRegionalOffice =
    searchParams.get("mstc_regional_office") || "";
  ...
  const listingType =
    (searchParams.get("listingType") as AuctionFilterParams["listingType"]) ||
    undefined;
  ...
  const sortBy =
    (searchParams.get("sortBy") as AuctionFilterParams["sortBy"]) || "newest";
  ...
  const loadMstcData = useCallback(async () => {
    ...
    try {
      const data = await MstcSearchService.searchMarketplaceCatalog(
        searchQuery,
        {
    ...
    selectedMstcSubcategory,
    selectedMstcLocation,
    selectedMstcSeller,
    selectedMstcRegionalOffice,
    startDate,
    endDate,
  ]);
      ...
      <div className="relative bg-slate-900 overflow-hidden py-12">
        ...
        <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-slate-400 mb-6">
            Browse live commercial auctions and official government catalogs.
          </p>
          ...
          <div className="lg:w-1/4 shrink-0 lg:sticky lg:top-[96px] lg:max-h-[calc(100vh-120px)] lg:overflow-y-auto custom-scrollbar z-20">
            <AuctionFilters
              ...
              onFilterChange={
                activeTab === "commercial"
                  ? handleFilterChange
                  : handleMstcFilterChange
              }
              initialFilters={
                activeTab === "commercial"
                  ? filters
                  : {
                      categoryIds: selectedMstcCategory
                        ? [selectedMstcCategory]
                        : [],
              ...
              <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-6 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="hidden lg:block text-sm text-slate-600 font-medium">
                  {!isAnyFilterActive ? (
                    ...
                    <span>
                      Showing {auctions.length > 0 ? (page - 1) * limit + 1 : 0}{" "}
                    ...
                    >
                      {auctions.map((auction) => (
                        <AuctionCard
                          ...
                          auction={auction}
                          isGrid={isGridView}
                          isWatchlistedInitial={watchlistIds.includes(
                            auction.id,
                          )}
                        />
                      ))}
          ...
          <div className="relative w-full max-w-4xl h-[90vh] md:h-[80vh] bg-white rounded-3xl overflow-hidden shadow-2xl flex flex-col border border-slate-200 animate-scale-up animate-duration-200">
            ...
            <div className="px-6 py-4.5 border-b border-slate-150 flex justify-between items-center bg-slate-50/50">
              <div className="flex items-center gap-2.5">
                <span className="text-sm font-semibold text-slate-400 font-mono">
                  Ref:{" "}
                  {selectedPreviewItem.mstc_auction_number.split("/").pop()}
                </span>
              </div>
              <button
            ...
            <div className="flex-grow flex flex-col md:flex-row overflow-hidden">
              ...
              <div className="flex-grow overflow-y-auto p-6 space-y-6 bg-slate-50/25">
                ...
                <div>
                  <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 font-mono">
                    Category / Item Type
                  </h4>
                  {(() => {
                    const parts =
                      selectedPreviewItem.category_name.split(" | ");
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                  ...
                  <div className="md:col-span-5 bg-white rounded-2xl p-4 border border-slate-200 shadow-2xs space-y-1.5">
                    ...
                    <div className="font-mono text-sm text-slate-700 break-all select-all flex justify-between items-center bg-slate-50/50 p-3 rounded-lg border border-slate-100">
                      <span className="mr-2 text-[13px] font-bold leading-snug">
                        {selectedPreviewItem.mstc_auction_number}
                      </span>
                      <button
                        onClick={() => {
                  ...
                  <div className="md:col-span-4 bg-white rounded-2xl p-4 border border-slate-200 shadow-2xs space-y-2.5">
                    <div className="flex flex-col">
                      ...
                      <span className="text-sm font-bold text-slate-800 leading-tight mt-0.5">
                        {(() => {
                          const parts =
                            selectedPreviewItem.mstc_auction_number.split("/");
                          const rawOffice =
                            parts.length > 1 &&
                            parts[0].toUpperCase() === "MSTC"
                              ? parts[1]
                              : selectedPreviewItem.seller_name;
                    ...
                    {selectedPreviewItem.location && (
                      <div className="flex flex-col border-t border-slate-100 pt-2">
                        ...
                        <span className="text-sm font-bold text-slate-800 mt-0.5">
                          {expandMstcOffice(selectedPreviewItem.location)}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="md:col-span-3 bg-white rounded-2xl p-4 border border-slate-200 shadow-2xs space-y-2">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-semibold text-slate-800">
                        {new Date(
                          selectedPreviewItem.opening_date,
                    ...
                    <div className="flex justify-between text-xs border-t border-slate-100 pt-1.5">
                      ...
                      <span className="font-semibold text-slate-800">
                        {(() => {
                          ...
                          const biddingStartDate = new Date(
                            auctionDate.getTime() - 14 * 24 * 60 * 60 * 1000,
                          );
                          return biddingStartDate.toLocaleDateString(
                            undefined,
                            { dateStyle: "medium" },
                          );
                        })()}
                    ...
                    <div className="flex justify-between text-xs border-t border-slate-100 pt-1.5 items-center">
                      ...
                      {(() => {
                        ...
                        const now = new Date();
                        const diffMs =
                          biddingStartDate.getTime() - now.getTime();
                        if (diffMs <= 0) {
                          return (
                            <span className="font-bold text-xs px-2.5 py-0.5 rounded-md border border-emerald-200 text-emerald-700 bg-emerald-50">
                              Bidding Started
                            </span>
                          );
                        }
                ...
                <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs space-y-4">
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider font-mono border-b border-slate-100 pb-2 flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-slate-405 font-medium normal-case font-sans">
                      {generateCatalogSummary(selectedPreviewItem).items.length}{" "}
                      lots identified
                    </span>
                  </h4>
                  ...
                  <div className="overflow-x-auto rounded-xl border border-slate-150 bg-white">
                    <table className="w-full text-left border-collapse text-xs">
                      ...
                      <tbody className="divide-y divide-slate-100 text-slate-700">
                        {generateCatalogSummary(selectedPreviewItem).items.map(
                          (row) => (
                    ...
                    <div className="space-y-2.5 text-xs">
                      <div className="flex justify-between items-center bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                        ...
                        <span className="font-bold text-slate-800">
                          {
                            generateCatalogSummary(selectedPreviewItem)
                              .depositDetails.emd
                          }
                        </span>
                      </div>
                ...
                <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs space-y-3.5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {generateCatalogSummary(
                      ...
                      <div
                        key={i}
                        className="bg-slate-50/50 border border-slate-150 p-3.5 rounded-xl space-y-1"
                      >
                ...
                return (
                  <div className="w-full md:w-[320px] shrink-0 border-t md:border-t-0 md:border-l border-slate-200 bg-slate-50 p-5 overflow-y-auto flex flex-col space-y-5">
                    ...
                    <div className="space-y-2">
                      ...
                      summary.extracted_images.length > 0 ? (
                    ...
                    {summary.preview_image_url && (
                      <div className="space-y-2">
                        ...
                        <div className="relative rounded-2xl overflow-hidden border border-slate-200 shadow-2xs bg-white group">
                          <a
                            href={summary.preview_image_url}
                            target="_blank"
                            rel="noreferrer"
