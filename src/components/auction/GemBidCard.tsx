...
export const GemBidCard = memo(function GemBidCard({
  ...
  // Renders authentic official GeM Bid Document preview (matching official GeM portal document)
  const renderDocumentSheetPreview = () => {
    const datedStr = formatSimpleDate(item.created_at || item.start_date);
    const endDateTimeStr = formatGovDate(item.end_date);
    const startDateTimeStr = formatGovDate(item.start_date);
    const ministryName = item.ministry || (item.state ? `Govt of ${item.state}` : 'Ministry of Defence');
    const departmentName = item.department || item.department_name || 'Department of Military Affairs';
    ...
    return (
      <div 
        ...
        title="Click to view official GeM Bid Document"
      >
        ...
        <div className="w-full h-full border border-slate-900 bg-white p-2 flex flex-col justify-between overflow-hidden">
          ...
          <div className="border border-slate-800 text-[6.5px] font-sans bg-white divide-y divide-slate-800">
            ...
            <div className="flex divide-x divide-slate-800">
              <span className="font-bold w-[125px] shrink-0 px-1 py-0.5 text-slate-900 bg-white leading-tight">
                बिड खुलने की तारीख/समय /Bid Opening Date/Time
              </span>
              <span className="px-1 py-0.5 text-slate-900 font-mono font-semibold truncate leading-tight flex items-center">
                {startDateTimeStr}
              </span>
