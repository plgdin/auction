      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 bg-muted rounded border border-dashed border-border">
            <h3 className="text-lg font-bold text-foreground">No active auctions at the moment.</h3>
            <p className="mt-2 text-muted-foreground">Please check back later or subscribe to our notices.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {auctions.map((auction) => (
              <div key={auction.id} className="bg-white rounded-lg shadow-sm border border-border overflow-hidden hover:shadow-md transition-shadow flex flex-col">
                <div className="h-48 bg-muted relative">
                  {/* Placeholder for Auction Image */}
                  <div className="absolute inset-0 flex items-center justify-center text-muted-foreground">
                    Image Preview
                  </div>
                  <div className="absolute top-4 right-4 bg-green-500 text-white text-xs font-bold px-3 py-1 rounded uppercase tracking-wide shadow-sm">
                    Live
                  </div>
                </div>
                <div className="p-6 flex-grow flex flex-col">
                  <h3 className="text-xl font-bold text-foreground mb-2 line-clamp-2">{auction.title}</h3>
                  <p className="text-muted-foreground text-sm mb-4 line-clamp-2">{auction.description || 'No description provided.'}</p>
                  
                  <div className="mt-auto space-y-3">
                    <div className="flex items-center text-foreground font-medium">
                      <IndianRupee className="w-5 h-5 mr-2 text-muted-foreground" />
                  ...
                  <Link 
                    ...
                    className="mt-6 w-full inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded shadow-sm text-white bg-primary hover:bg-primary/90 transition-colors"
                  >
                    View Details
                  </Link>
                </div>
